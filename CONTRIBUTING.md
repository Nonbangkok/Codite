...
Please refer to the `README.md` and `PLANNING.md` files for details on the architecture and the current technical stack (Rust backend, Tree-sitter, React/Svelte frontend).

1.  Make sure you have Rust (`cargo`) and Node.js (`npm` or `pnpm`) installed.
2.  Follow the setup instructions in the respective `analyzer/` and `ui/` directories.

## Coding Style
*   **Rust:** Follow standard `rustfmt` formatting.
*   **Frontend:** Follow standard Prettier/ESLint rules for React/Svelte.
